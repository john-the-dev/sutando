...
- `[dm-only]` — privacy guard: forces delivery to the owner's DM and suppresses any `[channel:]` redirect on the same body (regardless of marker order), so content carrying private data (the morning briefing's calendar + email, etc.) can never land in a shared channel. Stripped before delivery and before voice speaks it. Parsed by `result_markers.parse_markers`.
