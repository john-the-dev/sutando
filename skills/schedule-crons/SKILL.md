...
0. **Session init (fresh-session only).** Check `state/proactive-loop-started.sentinel` under `${SUTANDO_WORKSPACE:-$HOME/.sutando/workspace}/`. If absent → `mkdir -p` the workspace `state/` and `touch` the sentinel. If present → this is a cron-driven re-invocation within an already-running session; skip this step and proceed. The sentinel is cleared by the SessionEnd hook in `src/session-handoff.sh` so the next fresh session re-runs this block.

   **Lazy catchup** — do NOT invoke `/catchup-after-startup` unconditionally on every cold start. `session-handoff.sh` + `session-state.md` + `/resume` already cover PRs, in-flight tasks, build log, and recent commits. The one unique slice is voice/phone conversation content from `conversation.sqlite`, and only when the owner's first message implies continuity with a prior voice/phone session (e.g. "what were we talking about?", "continue that", "what did I say earlier?"). In that case, and only in that case, invoke `/catchup-after-startup` to pull sqlite context. Per [#1388](https://github.com/sonichi/sutando/issues/1388): sonichi's recommendation (b) — make it lazy rather than unconditionally burning tokens on every fresh start.
