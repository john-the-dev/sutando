...
- **Chi's fleet** (auto-detected via env `SUTANDO_FLEET_OWNER=chi`): the
  script sets repo-local `user.email=4250911+sonichi@users.noreply.github.com`
  and `user.name=Chi Wang` inside the fresh clone so internal demo
  submissions don't need an extra config step.
