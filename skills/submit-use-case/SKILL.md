...
5. **Open a PR** (unless `--issue-only`).
   ...
   - Identity: **only** set repo-local `user.email` / `user.name` to
     ...
     is on Chi's fleet (detected via env `SUTANDO_FLEET_OWNER=chi`). For
     OSS submitters the script leaves git config alone so their own
     identity flows through — that's the whole point of CLA-Assistant
     signing.
