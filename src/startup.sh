...
if _RELAY_ENV="$(bash "$REPO/scripts/sutando-config.sh" claude-home-path channels/ag2space/.env)"; \
  ...
  # Ask launchd about its OWN job, never argv: `launchctl list` prints the job's
  # live PID, or `-` when loaded but not running. Every named-instance bridge
  ...
  if [ "$_gw_supervised" = "0" ]; then
    ...
    # duplicate. A pgrep guard here suppressed restart of a dead primary for as
    # long as any secondary was alive, since both share one argv.
