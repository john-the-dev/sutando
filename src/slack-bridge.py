...
def _write_task(event: dict, prefix: str, text: str, username: str | None) -> str | None:
    ...
    if access_tier == "owner" and (_notify_py.exists() or _transcribe_py.exists()):
        ...
        if _notify_py.exists():
            # Thread the progress ack under the original message when the task
            # came from a thread (thread_ts set — channel @mentions). Without
            # baking --thread-ts into the hint, an owner following it verbatim
            # posts the ack to the parent channel even though the task file
            # carries thread_ts (qingyun-wu review on #1889).
