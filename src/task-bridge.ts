	...
	setInterval(() => {
		...
		try {
			...
			for (const file of files) {
				...
				// src/result_markers.py) — it forces DM delivery and suppresses a
				// [channel:] redirect on the Python bridge side. It has no meaning
				// for the voice/task path, so strip it on read: this keeps voice
				// from ever speaking "dm only" and keeps it out of logs. Parity
				// with Python parse_markers(), which strips it before delivery.
