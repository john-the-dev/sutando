#!/usr/bin/env python3
"""Tests: the Slack bridge threads the FIRST reply (progress ping), not just the result.

Bug: the progress ping is sent by task-progress/notify.py, and the bridge wrote
that command into the task file with --channel-id but no --thread-ts. So the
first reply landed in the channel root while the result — which threads via
pending_replies[thread_ts] — replied in-thread, splitting the two apart.

Fix: bake `--thread-ts <ts>` into the NOTIFY FIRST command and carry a
`reply_thread_ts:` task-file field, both only for channel @mentions (thread_ts
set), not DMs (top-level).

Contract:
  (a) Channel @mention (thread_ts derived from event ts) → the task file's
      notify command includes `--thread-ts <ts>` AND a `reply_thread_ts: <ts>`
      line.
...
def test_mention_threads_notify_and_field():
    """(a) Channel @mention → --thread-ts in notify cmd + reply_thread_ts field."""
    with tempfile.TemporaryDirectory() as td:
        ...
        assert "reply_thread_ts: 1700000000.000001" in body, (
            f"task file must carry reply_thread_ts for a mention; body:\n{body}"
        )


def test_dm_stays_top_level():
    """(b) DM (channel_type == im) → neither --thread-ts nor reply_thread_ts."""
    with tempfile.TemporaryDirectory() as td:
        tasks = _setup_state(td)
        ...
        assert "reply_thread_ts:" not in body, (
            f"DM task file must NOT carry reply_thread_ts; body:\n{body}"
        )
