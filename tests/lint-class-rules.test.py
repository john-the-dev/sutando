...
class TestRunLintNoOp(unittest.TestCase):
    ...
    def test_exits_zero_on_main_without_migrate_sh(self):
        """The script must exit 0 on main where sutando-migrate.sh is absent."""
        result = subprocess.run(
            [sys.executable, str(LINT_SCRIPT)],
            capture_output=True, text=True
        )
        self.assertEqual(result.returncode, 0,
                         f"lint script must exit 0 when migrate.sh absent:\n{result.stdout}\n{result.stderr}")
        self.assertIn("SKIP", result.stdout,
                      "must print SKIP when migrate.sh absent")
