...
#      GUARDED: this fast path ALSO requires a real whitelist to exist first (either
#      the `.git/info/exclude` marker OR the pre-#1459 in-tree `.gitignore` deny-all),
#      so a half-initialized repo — HEAD parked on `host/<hostname>` but with NO
#      carrier-set filter — cannot ride the branch name past the guard into `git add -A`.
#
...
_assert_sync_initialized() {
    ...
    #
    # BUT only when a real whitelist already exists — otherwise a half-initialized
    # repo (HEAD parked on `host/<hostname>` but no carrier-set filter) would ride the
    # branch name straight into `git add -A` and commit the WHOLE workspace tree.
    # The exclude marker is checked above (returns 0 when present); here we also accept
    # the pre-#1459 in-tree `.gitignore` deny-all, which was that era's whitelist and is
    # exactly what a genuine pre-wsId workspace carries (see _migrate_flat_branch).
    local _cur_branch
